...
const edges = [
  { id: "edge-0-10", d: "M70 273L182 186", key: "0-10" },
  { id: "edge-0-20", d: "M70 328L185 451", key: "0-20" },
  { id: "edge-0-30", d: "M81 299L356 200", key: "0-30" },
  { id: "edge-10-30", d: "M260 174.109L349 173.991", key: "10-30" },
  { id: "edge-20-40", d: "M233 447L330 346", key: "20-40" },
  { id: "edge-40-60", d: "M393 318L555 323", key: "40-60" },
  { id: "edge-20-50", d: "M248 482L485 487", key: "20-50" },
  { id: "edge-50-70", d: "M563 491L735 493", key: "50-70" },
  { id: "edge-80-90", d: "M689 199L799 289", key: "80-90" },
  { id: "edge-20-60", d: "M248 468L563 351", key: "20-60" },
  { id: "edge-60-80", d: "M604 296L643 207", key: "60-80" },
  { id: "edge-70-90", d: "M789 461L825 354", key: "70-90" },
  { id: "edge-30-80", d: "M617 173L429 174", key: "30-80" },
];
...
const SvgComponent = ({ simData }) => {
  ...
  if (match) {
    const early = match.early;      
    const latest = match.latest;    
    const slack = match.slack !== undefined ? match.slack : latest - early;
    return { ...node, early, latest, slack };
  }
  return node;
});
  ...
  return (
    <svg width="700" height="600" viewBox="0 0 1000 600" fill="none">
     {/* Aristas */}
{edges.map((edge) => {
  const match = edge.d.match(/M(\d+(?:\.\d+)?) (\d+(?:\.\d+)?)L(\d+(?:\.\d+)?) (\d+(?:\.\d+)?)/);
  let midX = 0, midY = 0;
  if (match) {
  ...
  const time = simData?.tiemposPert?.[edge.key] ?? "";
  const yOffset = 15;
  ...
  return (
    <g key={edge.id}>
      <path
        id={edge.id}
        d={edge.d}
        stroke={isCritical ? "red" : stroke_color}
        strokeWidth={isCritical ? 4 : stroke_width}
      />
      {time && (
        <text
          x={midX}
            ...
            return (
              <g key={type} id={`${type}_${node.id}`}>
                ...
                <text
                  ...
                  fill={type === "slack" ? "darkgreen" : "black"}
                >
                  {node[type]}
                </text>
              </g>
